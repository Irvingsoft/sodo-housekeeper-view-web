<template>
  <el-row>
    ...
    <el-col :span="15">
      <basic-container>
        <el-button-group>
          <el-button v-if="permission.region_add" icon="el-icon-circle-plus-outline" size="small" type="primary"
                     @click="addChildren">新增下级
          </el-button>
          <el-button v-if="permission.region_delete" icon="el-icon-delete" size="small" type="primary"
                     @click="handleDelete">删除
          </el-button>
          <el-button v-if="permission.region_debug" icon="el-icon-video-play" size="small" type="primary"
                     @click="handleDebug">调试
          </el-button>
        </el-button-group>
      </basic-container>
      <basic-container>
        <avue-form ref="form" v-model="regionForm" :option="regionOption" @submit="handleSubmit">
          <template slot="code" slot-scope="{}">
            <el-input v-model="regionForm.subCode" placeholder="请输入 区划子编号">
              <template slot="prepend">{{ regionForm.parentCode }}</template>
            </el-input>
          </template>
        </avue-form>
        <el-dialog :visible.sync="debugBox"
                   append-to-body
                   title="行政区划数据调试"
                   width="350px">
          <avue-form v-model="debugForm" :option="debugOption"/>
        </el-dialog>
      </basic-container>
    </el-col>
  </el-row>
...
<script>
import {getDetail, getLazyTree, remove, submit} from "@/api/base/region";
